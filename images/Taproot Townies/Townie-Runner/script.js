(() => {
  ...
  function updateStats() {
    $("gamesPlayed").textContent = stats.games.toLocaleString();
    $("totalCoins").textContent = stats.coins.toLocaleString();
    $("bestScore").textContent = stats.best.toLocaleString();
  ...
  function openGame() {
    ...
    $("overlayText").textContent = "Press Space, Up, click, or tap to jump.";
  ...
  function jump() {
    ...
    game.player.velocityY = canGroundJump ? -760 : -700;
  ...
  function isAreaClear(candidate, padding = 18) {
  ...
  function addCoins(baseX, baseY, count = 4, arc = false) {
    ...
    for (let i = 0; i < count; i += 1) {
      const coin = {
        x: baseX + i * spacing,
        y: baseY - (arc ? Math.sin((i / Math.max(1, count - 1)) * Math.PI) * 55 : 0),
        width: 26,
        height: 26,
        collected: false,
  ...
  function addObstacle() {
    ...
    if (roll < 0.22 && game.score > 900) {
      const platform = { x, y: game.groundY - 118, width: rand(150, 220), height: 16 };
      if (isAreaClear(platform, 25)) {
        game.platforms.push(platform);
        addCoins(platform.x + 25, platform.y - 40, 3, false);
      }
    } else {
      const type = roll < 0.48 ? "rock" : roll < 0.76 ? "crystal" : "drone";
      const specs = {
        rock: { width: 44, height: 46, y: game.groundY - 46 },
        crystal: { width: 32, height: 70, y: game.groundY - 70 },
        drone: { width: 56, height: 30, y: game.groundY - 118 }
      }[type];
    ...
    if (chance(0.72)) addCoins(x + rand(70, 120), game.groundY - rand(68, 105), chance(0.45) ? 5 : 3, chance(0.6));
    game.nextSpawn = Math.max(0.72, 1.45 - game.score / 18000) + rand(0.1, 0.45);
  }
  ...
  function addRecruitable() {
    ...
    if (!available.length) {
      game.nextFollower = rand(7, 13);
      return;
    }
    const townieIndex = available[Math.floor(Math.random() * available.length)];
    const recruit = {
      x: canvas.width + 45,
      y: game.groundY - 54,
      width: 48,
    ...
    if (isAreaClear(recruit, 35)) game.recruitables.push(recruit);
    game.nextFollower = rand(7, 13);
  }

  function addDoor() {
    const door = { x: canvas.width + 50, y: game.groundY - 92, width: 58, height: 92, pulse: 0 };
    if (isAreaClear(door, 42)) game.doors.push(door);
    game.nextDoor = rand(15, 24);
  }

  function addExtraLife() {
    const life = { x: canvas.width + 50, y: game.groundY - rand(82, 125), width: 34, height: 34, bob: 0 };
  ...
  function addPower() {
    ...
    const power = { x: canvas.width + 50, y: game.groundY - rand(88, 130), width: 36, height: 36, ...data, bob: 0 };
  ...
  function update(deltaTime) {
    ...
    game.speed = Math.min(760, 335 + game.score * 0.014);
    ...
    const leaderStartX = 100;
    const shiftPerFollower = 18;
  ...
  function drawBackground() {
    ...
    sky.addColorStop(0.62, env.sky2);
    sky.addColorStop(0.625, env.horizon);
    ...
    ctx.beginPath();
    ctx.arc(790, 80, 42, 0, Math.PI * 2);
    ctx.fill();

    ctx.fillStyle = "#ffffff";
    const offset = (game.elapsed * game.speed * 0.025) % canvas.width;
    const stars = [[100, 62], [190, 112], [300, 54], [430, 95], [555, 45], [680, 130], [880, 48]];
    stars.forEach(([x, y]) => ctx.fillRect((x - offset + canvas.width) % canvas.width, y, 2, 2));

    ctx.strokeStyle = env.accent;
    ctx.lineWidth = 4;
    ctx.beginPath();
    ctx.moveTo(0, game.groundY + 2);
    ctx.lineTo(canvas.width, game.groundY + 2);
    ctx.stroke();
  }

  function drawTownie(imageIndex, x, y, width, height, frame, alpha = 1) {
  ...
  function draw() {
    ...
    game.extraLives.forEach((life) => {
      ctx.font = "30px Arial";
      ctx.fillText("\u{2764}\u{FE0F}", life.x, life.y + Math.sin(life.bob) * 4 + 29);
    });
      ...
      const icon = power.key === "shield"
        ...
        : power.key === "magnet"
          ...
          : power.key === "doubleJump"
            ...
            : "\u{26A1}";
      ctx.font = "30px Arial";
      ctx.fillText(icon, power.x, power.y + Math.sin(power.bob) * 4 + 29);
    });
    ...
    game.doors.forEach((door) => {
      ...
      ctx.font = "28px Arial";
      ctx.fillText("\u{1F6AA}", door.x + 14, door.y + 51);
      ctx.shadowBlur = 0;
    });
    ...
    game.recruitables.forEach((recruit) => {
      drawTownie(recruit.townieIndex, recruit.x, recruit.y, recruit.width, recruit.height, recruit.frame);
      ctx.font = "17px Arial";
      ctx.fillText("\u{2757}", recruit.x + 13, recruit.y - 8);
    });
