...
(() => {
  ...
  const preRunPanel = document.getElementById("preRunPanel");
  const preRunStartBtn = document.getElementById("preRunStartBtn");
  const cabinetScreen = document.getElementById("cabinetScreen");
  const gameSubheader = document.getElementById("gameSubheader");

  const leaderboardBtn = document.getElementById("leaderboardBtn");
  const leaderboardHomeBtn = document.getElementById("leaderboardHomeBtn");
  const leaderboardPlayBtn = document.getElementById("leaderboardPlayBtn");
  const gameHomeBtn = document.getElementById("gameHomeBtn");
  const leaderboardPanel = document.getElementById("leaderboardPanel");
  const leaderboardList = document.getElementById("leaderboardList");
  const playerNameInput = document.getElementById("playerName");
  const owlPrevBtn = document.getElementById("owlPrevBtn");
...
let leaderboardOwlLoopHandle = null; // leaderboard owl animation loop
...
function cancelAllLoops() {
  ...
  const sounds = {
    ...
    shoot_btc:  new Audio("assets/sounds/shoot_btc.wav"),
    shoot_eth:  new Audio("assets/sounds/shoot_eth.wav"),
    shoot_doge: new Audio("assets/sounds/shoot_doge.wav"),
    shoot_xrp:  new Audio("assets/sounds/shoot_xrp.wav"),
    shoot_sol:  new Audio("assets/sounds/shoot_sol.wav"),
    bonus:      new Audio("assets/sounds/bonus.wav"),
  ...
  const owlJSONNode = document.getElementById("owl-json");
  const owlIMGNode  = document.getElementById("owl-img");

  const OWL_OPTIONS = [
  { id: "owl0", name: "Nudy Judy", json: "assets/owl0.json", img: "assets/owl0.png" },
  { id: "owl1", name: "Mateo", json: "assets/owl1.json", img: "assets/owl1.png" },
  { id: "owl2", name: "Ronald", json: "assets/owl2.json", img: "assets/owl2.png" },
  { id: "owl3", name: "Charmin", json: "assets/owl3.json", img: "assets/owl3.png" },
  { id: "owl4", name: "Wayne", json: "assets/owl4.json", img: "assets/owl4.png" },
  { id: "owl5", name: "Desi", json: "assets/owl5.json", img: "assets/owl5.png" },
  { id: "owl6", name: "Johnathan", json: "assets/owl6.json", img: "assets/owl6.png" },
  { id: "owl7", name: "Rambo", json: "assets/owl7.json", img: "assets/owl7.png" },
  { id: "owl8", name: "Drew", json: "assets/owl8.json", img: "assets/owl8.png" },
  { id: "owl9", name: "Doogie", json: "assets/owl9.json", img: "assets/owl9.png" },
  { id: "owl10", name: "Mikey", json: "assets/owl10.json", img: "assets/owl10.png" },
  { id: "owl11", name: "Kenji", json: "assets/owl11.json", img: "assets/owl11.png" },
  { id: "owl12", name: "Otis", json: "assets/owl12.json", img: "assets/owl12.png" },
  { id: "owl13", name: "George", json: "assets/owl13.json", img: "assets/owl13.png" },
  { id: "owl14", name: "Eddie", json: "assets/owl14.json", img: "assets/owl14.png" },
  { id: "owl15", name: "Cedrick", json: "assets/owl15.json", img: "assets/owl15.png" },
  { id: "owl16", name: "Romeo", json: "assets/owl16.json", img: "assets/owl16.png" },
  { id: "owl17", name: "Petey", json: "assets/owl17.json", img: "assets/owl17.png" },
  { id: "owl18", name: "Zane", json: "assets/owl18.json", img: "assets/owl18.png" },
  { id: "owl19", name: "Robin", json: "assets/owl19.json", img: "assets/owl19.png" },
  { id: "owl20", name: "Sunny", json: "assets/owl20.json", img: "assets/owl20.png" }
];

let selectedOwlId = "owl0";
...
function randBonusDelay() {
  ...
  bindHoldControl(mobileFireBtn, "Space");
  ...
  async function loadOwl(){
    const cfg = await fetch(owlJSONNode.value).then(r => r.json());
    const img = await new Promise((res, rej) => { const i=new Image(); i.onload=()=>res(i); i.onerror=()=>rej(new Error("Failed to load owl image")); i.src = owlIMGNode.value; });
...
function populateOwlSelector() {
  updateOwlName();
}
	
function updateOwlName() {
  const owl = OWL_OPTIONS.find(o => o.id === selectedOwlId);
  if (owlNameEl && owl) {
    owlNameEl.textContent = owl.name.toUpperCase();
  }
}	

function applySelectedOwl(owlId) {
  const owl = OWL_OPTIONS.find(o => o.id === owlId);
  if (!owl) return;
  selectedOwlId = owl.id;
  owlJSONNode.value = owl.json;
  owlIMGNode.value = owl.img;
  updateOwlName();
}

function changeOwl(direction) {
  const currentIndex = OWL_OPTIONS.findIndex(o => o.id === selectedOwlId);
  if (currentIndex === -1) return;

  let nextIndex = currentIndex + direction;
  if (nextIndex < 0) nextIndex = OWL_OPTIONS.length - 1;
  if (nextIndex >= OWL_OPTIONS.length) nextIndex = 0;

  applySelectedOwl(OWL_OPTIONS[nextIndex].id);
  loadOwl().then(() => {
    if (owlPreviewSprite) {
      const names = Object.keys(owlPreviewSprite.animations);
      const startAnim = names.includes("idle_down")
        ? "idle_down"
        : names.includes("idle_up")
        ? "idle_up"
        : names[0];
      owlPreviewSprite.set(startAnim);
    }
...
const owlAssetCache = new Map();

async function getOwlAsset(owlId) {
  if (owlAssetCache.has(owlId)) return owlAssetCache.get(owlId);

  const owl = OWL_OPTIONS.find(o => o.id === owlId);
  if (!owl) return null;

  const cfg = await fetch(owl.json).then(r => r.json());
  ...
  const asset = { cfg, img };
  owlAssetCache.set(owlId, asset);
  return asset;
}

async function drawLeaderboardOwls() {
  const canvases = leaderboardList.querySelectorAll(".leaderboard-owl-canvas");

  for (const c of canvases) {
    const owlId = c.dataset.owl;
    const asset = await getOwlAsset(owlId);
    if (!asset) continue;
    ...
    const animName = names.includes("idle_down")
      ? "idle_down"
      : names.includes("idle_up")
      ? "idle_up"
      : names[0];
    ...
    // animate between the available idle_down frames
    const totalFrames = anim.length || 1;
    const frameIndex = totalFrames > 1
      ? Math.floor(performance.now() / 300) % totalFrames
      : 0;
    ...
    // bump it up slightly so it lines up better with the text row
    const dx = (c.width - dw) / 2;
    const dy = (c.height - dh) / 2;

    ctx2.imageSmoothingEnabled = false;
    ctx2.drawImage(img, sx, sy, frameW, frameH, dx, dy, dw, dh);
  }

  // keep animating while leaderboard is visible
  if (!leaderboardPanel.classList.contains("hidden")) {
    leaderboardOwlLoopHandle = requestAnimationFrame(drawLeaderboardOwls);
  }
}
	
function getLeaderboard() {
  try {
    return JSON.parse(localStorage.getItem("owlsLeaderboard") || "[]");
  } catch (e) {
    return [];
  }
}
...
function saveLeaderboard(entries) {
  localStorage.setItem("owlsLeaderboard", JSON.stringify(entries));
}

function addScoreToLeaderboard() {
  const entries = getLeaderboard();
  entries.push({
    name: currentPlayerName || "PLAYER 1",
    owl: selectedOwlId,
    score,
    level,
...
function renderLeaderboard() {
  ...
  .map((entry, index) => `
    <div class="leaderboard-row">
      ...
      <div class="leaderboard-owl-cell">
        <canvas class="leaderboard-owl-canvas" data-owl="${entry.owl}" width="48" height="48"></canvas>
      </div>
    </div>
  `)
  .join("");
...
function showLeaderboard() {
  ...
  if (preRunPanel) {
    preRunPanel.classList.add("hidden");
    preRunPanel.style.display = "none";
  }
  ...
  // show the leaderboard as its own page
  leaderboardPanel.classList.remove("hidden");
  leaderboardPanel.style.display = "block";

  // start owl animation loop
  if (leaderboardOwlLoopHandle) cancelAnimationFrame(leaderboardOwlLoopHandle);
  leaderboardOwlLoopHandle = requestAnimationFrame(drawLeaderboardOwls);
}
	
function hideLeaderboard() {
  // stop owl animation loop
  if (leaderboardOwlLoopHandle) {
    cancelAnimationFrame(leaderboardOwlLoopHandle);
    leaderboardOwlLoopHandle = null;
  }
  ...
  // bring back the main menu panel
  if (mainMenuPanel) {
  ...
  function setGameChrome(mode = "hidden") {
    if (!cabinetScreen) return;

    const gameplay = mode === "gameplay";
    const prerun = mode === "prerun";

    cabinetScreen.classList.toggle("gameplay-active", gameplay);
    cabinetScreen.classList.toggle("prerun-active", prerun);

    if (gameSubheader) {
      gameSubheader.hidden = !(gameplay || prerun);
    }
  }
  ...
  function showPreRun() {
    ...
    if (preRunPanel) {
      preRunPanel.classList.remove("hidden");
      preRunPanel.style.display = "block";
    }

    overlay.style.display = "grid";
    window.setTimeout(() => playerNameInput?.select(), 40);
  ...
  window.addEventListener("keydown", e=>{
    ...
    if (e.code==="Enter") {
      if (gameOver) {
        startGame();
      } else if (!running && preRunPanel && !preRunPanel.classList.contains("hidden")) {
        startGame();
      } else if (!running && mainMenuPanel && !mainMenuPanel.classList.contains("hidden")) {
        showPreRun();
      }
    }
  });
  ...
  startBtn?.addEventListener("click", showPreRun);
  preRunStartBtn?.addEventListener("click", startGame);
  owlPrevBtn?.addEventListener("click", () => changeOwl(-1));
  owlNextBtn?.addEventListener("click", () => changeOwl(1));
...
leaderboardPlayBtn?.addEventListener("click", () => {
  showPreRun();
});
...
function buildBunkers(force = false) {
  ...
  function returnToGameHome() {
    ...
    // IMPORTANT: hide the pre-run panel before restoring Home.
    // Without this, both panels remain visible inside #overlay and
    // each panel gets its own scrollbar.
    if (preRunPanel) {
      preRunPanel.classList.add("hidden");
      preRunPanel.style.display = "none";
    }
  ...
  // ---------- Game Flow ----------
  function startGame(){
	currentPlayerName = (playerNameInput?.value || "PLAYER 1").trim() || "PLAYER 1";
    applySelectedOwl(selectedOwlId || "owl0");
    cancelAllLoops();
    setGameChrome("gameplay");
